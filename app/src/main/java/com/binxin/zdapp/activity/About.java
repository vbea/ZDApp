...
import com.binxin.zdapp.classes.*;
...
public class About extends Activity
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		btn_new.setOnClickListener(new OnClickListener()
		{
			public void onClick(View v)
			{
				...
				{
					status = true;
					dialog = psd.show(About.this,null,"检查更新中…",false,true);
					dialog.show();
					...
					UmengUpdateAgent.setUpdateListener(new UmengUpdateListener()
					{
						...
						{
							showUpdate(updateStatus, updateInfo, status);
						}
					});
		...
		btn_web.setOnClickListener(new OnClickListener()
		{
			public void onClick(View v)
			{
				...
				Uri content_url = Uri.parse("http://vbea.wicp.net");
	...
	private void showUpdate(int updateStatus,UpdateResponse updateInfo,boolean state)
	{
		//ExceptionHandler.log("检查更新status:"+state);
		if (state)
		{
			switch (updateStatus)
			{
	...
	public void setMyTheme()
	{
		...
		if (Common.APP_THEME_ID != 19)
			MyThemes.setThemes(this,aboutLayout);
		else
		{
			MyThemes.setThemeColor(this);
			aboutLayout.setBackgroundResource(R.color.title);
		}

...
public class App_audiotest extends Activity 
{
	...
	public void onCreate(Bundle savedInstanceState) 
	{
		...
		btn_selectfile.setOnClickListener(new OnClickListener()
		{
			public void onClick(View v)
			{
				startActivityForResult(new Intent(App_audiotest.this, SelectAudio.class), 0);
			}
		});
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data)
	{
		if (resultCode == RESULT_OK)
		{
			...
			selectFilePath = b.getString("path");
			selectFilePath = selectFilePath.replace(Environment.getExternalStorageDirectory().getAbsolutePath(),"");
			selectFilePath = selectFilePath.substring(1);
			if (isPlay || isPause)
			{
				mediaPlayer.stop();

...
public class FilesCommon
{
	...
	public FilesCommon(Context context)
	{
		...
		if (Shipment.SHOW_THUMBNAIL)
		{
			...
			listFileId = new ArrayList<String>();
			getFileIdList();
		}
	}
	...
	public String getThumbPath(String filePath)
	{
		...
		{
			//10月12日增加，解决部分路径不能获得缩略图的问题
			for (String s : listFileId)
			{
				if (filePath.endsWith(s.replace(getSDCardPath(), "")))
					return getThumbPath(s);
			}
		}
		return null;
	}
	...
	private void getFileIdList()
	{
		Iterator itt = hasFileId.entrySet().iterator();
		while (itt.hasNext())
		{
			Map.Entry entry = (Map.Entry) itt.next();
			listFileId.add(entry.getKey().toString());
		}
	}
}
